...
class Cast(BaseAudit):
	...
	def __init__(self, num_candidates, num_winners, num_stages, batch_size, num_batches, risk_tolerance, threshold, random_seed):
		...
		self.alpha = calc_alpha_s(risk_tolerance)
	...
	def calc_adj_margin(self, winner, loser):
		...
		for num in self.unaudited:
			reported_margin = reported_margin + self.reported_batch_info[winner] - self.reported_batch_info[loser]

		for batches in self.audited_batch_info:
			audited_margin = audited_margin + self.audited_batch_info[winner] - self.audited_batch_info[loser]
	...
	def calc_u_ps(self):
		...
		for batch_num in self.unaudited:
			...
			for idw, winner in enumerate(self.winners):
				for idl, loser in enumerate(self.losers):
					u_p = (self.reported_batch_info[batch_num][winner] - self.reported_batch_info[batch_num][loser] + self.batch_size) / adj_margin[idw][idl]
					max_u_p.append(u_p)
			u_ps[batch] = np.amax(max_u_p)
		return u_ps
	...
	def run_audit(self):
		...
		for i in range(self.num_stages):
			T, squigglie_u_ps = self.calc_T(self)
			n = self.calc_n()
			batches_to_audit = random.sample(self.unaudited, n)
			for batch_num in batches_to_audit:
				np.delete(self.unaudited, batch_num)
				self.audited_batch_info[batch_num] = get_batch_info()
